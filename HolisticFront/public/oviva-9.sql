...
--
-- Servidor: localhost
-- Tiempo de generación: 28-05-2024 a las 11:14:05
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateQa` (IN `id_qa_param` SMALLINT, IN `question_param` VARCHAR(100), IN `answer_param` VARCHAR(1000))   BEGIN
    UPDATE qa 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateRecipe` (IN `id_recipe_param` SMALLINT, IN `title_param` VARCHAR(100), IN `image_param` VARCHAR(300), IN `description_param` VARCHAR(10000), IN `category_param` ENUM('Desayunos','Almuerzos','Meriendas','Cenas'))   BEGIN
    UPDATE recipe
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateVideo` (IN `id_video_param` SMALLINT, IN `title_param` VARCHAR(200), IN `link_param` VARCHAR(200), IN `category_param` ENUM('Yoga','Pilates'))   BEGIN
    UPDATE video 
    SET title = title_param,
        link = link_param,
        category = category_param
...
--
ALTER TABLE `person`
  MODIFY `id_person` smallint(6) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;

--
-- AUTO_INCREMENT de la tabla `qa`
--
ALTER TABLE `qa`
  MODIFY `id_qa` smallint(6) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17;
...
--
ALTER TABLE `user`
  MODIFY `id_user` smallint(6) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
